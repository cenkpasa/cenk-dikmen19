...
interface PersonnelContextType {
    ...
    getPayslipEntriesForUser: (userId: string, period: string) => PayslipEntry | undefined;
    addLeaveRequest: (request: Omit<LeaveRequest, 'id' | 'requestDate' | 'status'>, statusOverride?: LeaveRequest['status']) => Promise<void>;
...
export const PersonnelProvider = ({ children }: { children: ReactNode }) => {
    ...
    const payslipEntries = useLiveQuery(() => db.payslipEntries.toArray(), []) || [];
    ...
    const getPayslipEntriesForUser = useCallback((userId: string, period: string) => {
        return payslipEntries.find(p => p.userId === userId && p.period === period);
    }, [payslipEntries]);


    const addLeaveRequest = async (requestData: Omit<LeaveRequest, 'id' | 'requestDate' | 'status'>, statusOverride?: LeaveRequest['status']) => {
        if (!currentUser) return;
        const newRequest: LeaveRequest = {
            ...requestData,
            id: uuidv4(),
            requestDate: new Date().toISOString(),
            status: (currentUser.role === 'admin' && statusOverride) ? statusOverride : 'pending'
        };
        await db.leaveRequests.add(newRequest);
        const logMessage = statusOverride 
            ? `Admin manually added leave with status ${statusOverride}` 
            : 'User requested leave';
        await auditLogService.logAction(currentUser, 'CREATE_LEAVE_REQUEST', 'leaveRequest', newRequest.id, logMessage);
    };
