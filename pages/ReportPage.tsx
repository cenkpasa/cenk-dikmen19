...
const ReportControls = ({ filters, setFilters, isAdmin }: { filters: ReportFilters, setFilters: React.Dispatch<React.SetStateAction<ReportFilters>>, isAdmin: boolean }) => {
    ...
    const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'startDate' || name === 'endDate') {
            ...
            setFilters(prev => ({ ...prev, [name]: value as ReportType | string }));
        }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    ...
                    <select name="reportType" id="reportType" value={filters.reportType} onChange={handleFilterChange} className="w-full mt-1 p-2 border rounded-md">
                        ...
                        <option value="mileage_expense">{t('mileage_expense')}</option>
                        <option value="punctuality">{t('punctuality')}</option>
                    </select>
                </div>
                <div>
...
const ReportPage = () => {
    ...
    const { data, columns, title, summary, chartData } = useReportGenerator(filters);
     ...
     const ReportHeader = () => {
        if (filters.reportType === 'mileage_expense') {
            return (
                <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="p-3 bg-blue-100 rounded-md text-center">
                        <h4 className="text-sm font-semibold">{t('mileage_rate_per_km')}</h4>
                        <p className="text-xl font-bold">{formatCurrency(COMPANY_INFO.mileageRate, 'TRY')}</p>
                    </div>
                    <div className="p-3 bg-green-100 rounded-md text-center">
                        <h4 className="text-sm font-semibold">{t('total_distance')}</h4>
                        <p className="text-xl font-bold">{summary['Toplam Mesafe'] || 0} km</p>
                    </div>
                     <div className="p-3 bg-yellow-100 rounded-md text-center">
                        <h4 className="text-sm font-semibold">{t('total_reimbursement')}</h4>
                        <p className="text-xl font-bold">{formatCurrency(summary['Toplam Geri Ödeme'] || 0, 'TRY')}</p>
                    </div>
                </div>
            );
        }
