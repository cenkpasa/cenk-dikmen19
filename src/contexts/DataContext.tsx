...
interface DataContextType {
    ...
    addInterview: (interview: Omit<Interview, 'id' | 'createdAt' | 'userId'>) => Promise<void>;
    updateInterview: (interview: Interview) => Promise<void>;
    
    offers: Offer[];
    addOffer: (offer: Omit<Offer, 'id' | 'teklifNo' | 'createdAt' | 'userId'>) => Promise<void>;
    updateOffer: (offer: Offer) => Promise<void>;
    bulkAddOffers: (newOffersData: Omit<Offer, 'id' | 'createdAt' | 'teklifNo' | 'userId'>[]) => Promise<number>;

    technicalInquiries: TechnicalInquiry[];
    addTechnicalInquiry: (inquiry: Omit<TechnicalInquiry, 'id' | 'createdAt' | 'userId'>) => Promise<string>;
    updateTechnicalInquiry: (inquiry: TechnicalInquiry) => Promise<void>;
    deleteTechnicalInquiry: (inquiryId: string) => Promise<void>;
}
...
export const DataProvider = ({ children }: DataProviderProps) => {
    ...
    const appointments = useLiveQuery(() => 
        isUserAdmin || !currentUser ? db.appointments.toArray() : db.appointments.where('userId').equals(currentUser.id).toArray(),
        [isUserAdmin, currentUser?.id]
    ) || [];
    
    const interviews = useLiveQuery(() => 
        isUserAdmin || !currentUser ? db.interviews.toArray() : db.interviews.where('userId').equals(currentUser.id).toArray(),
        [isUserAdmin, currentUser?.id]
    ) || [];

    const offers = useLiveQuery(() => 
        isUserAdmin || !currentUser ? db.offers.toArray() : db.offers.where('userId').equals(currentUser.id).toArray(),
        [isUserAdmin, currentUser?.id]
    ) || [];

    const technicalInquiries = useLiveQuery(() => 
        isUserAdmin || !currentUser ? db.technicalInquiries.toArray() : db.technicalInquiries.where('userId').equals(currentUser.id).toArray(),
        [isUserAdmin, currentUser?.id]
    ) || [];
    ...
    const addCustomer = async (customerData: Omit<Customer, 'id' | 'createdAt'>): Promise<string> => {
        ...
        if (!isOnline) {
            await db.syncQueue.add({ type: 'add-customer', payload: newCustomer, timestamp: Date.now() });
            showNotification('offlineQueueAdd', 'info');
        }

        const newId = await db.customers.add(newCustomer);
        addNotification({
            messageKey: 'activityCustomerAdded', replacements: { name: newCustomer.name }, type: 'customer', link: { page: 'customers', id: newId }
        });
    ...
    const bulkAddCustomers = async (newCustomersData: Omit<Customer, 'id' | 'createdAt'>[]) => {
        ...
        for (const newCust of newCustomersData) {
            ...
            if (!isDuplicate) {
                customersToAdd.push({ ...newCust, id: uuidv4(), createdAt: new Date().toISOString(), synced: true });
            }
        }
    ...
    const addAppointment = async (appointmentData: Omit<Appointment, 'id' | 'createdAt'>) => {
        ...
        addNotification({
            messageKey: 'activityAppointmentAdded', replacements: { title: newAppointment.title }, type: 'appointment', link: { page: 'appointments' }
        });
    ...
    // Interview Actions
    const addInterview = async (interviewData: Omit<Interview, 'id' | 'createdAt' | 'userId'>) => {
        if (!currentUser) throw new Error("User not authenticated");
        ...
        addNotification({
            messageKey: 'activityInterviewAdded', replacements: { customer: customer?.name || '' }, type: 'interview', link: { page: 'gorusme-formu', id: newId as string }
        });
    ...
    // Offer Actions
    const addOffer = async (offerData: Omit<Offer, 'id'|'createdAt'|'teklifNo' | 'userId'>) => {
        if (!currentUser) throw new Error("User not authenticated");
        ...
        addNotification({
            messageKey: 'activityOfferAdded', replacements: { teklifNo: newOffer.teklifNo }, type: 'offer', link: { page: 'teklif-yaz', id: newId as string }
        });
    ...
    const bulkAddOffers = async (newOffersData: Omit<Offer, 'id'|'createdAt'|'teklifNo'|'userId'>[]) => {
        if (!currentUser) throw new Error("User not authenticated");
    ...
    // Technical Inquiry Actions
    const addTechnicalInquiry = async (inquiryData: Omit<TechnicalInquiry, 'id' | 'createdAt' | 'userId'>): Promise<string> => {
        if (!currentUser) throw new Error("User not authenticated");
        const newInquiry: TechnicalInquiry = {
            ...inquiryData,
            id: uuidv4(),
            userId: currentUser.id,
            createdAt: new Date().toISOString(),
        };
        const newId = await db.technicalInquiries.add(newInquiry);
        await auditLogService.logAction(currentUser, 'CREATE_INQUIRY', 'technicalInquiry', newId, `Inquiry '${newInquiry.title}' created.`);
        return newId;
    };

    const updateTechnicalInquiry = async (updatedInquiry: TechnicalInquiry) => {
        if (!currentUser) throw new Error("User not authenticated");
        await db.technicalInquiries.put(updatedInquiry);
        await auditLogService.logAction(currentUser, 'UPDATE_INQUIRY', 'technicalInquiry', updatedInquiry.id, `Inquiry '${updatedInquiry.title}' updated.`);
    };

    const deleteTechnicalInquiry = async (inquiryId: string) => {
        if (!currentUser || !isUserAdmin) {
            showNotification('permissionDenied', 'error');
            return;
        }
        const inquiryToDelete = technicalInquiries.find(i => i.id === inquiryId);
        await db.technicalInquiries.delete(inquiryId);
        await auditLogService.logAction(currentUser, 'DELETE_INQUIRY', 'technicalInquiry', inquiryId, `Inquiry '${inquiryToDelete?.title || inquiryId}' deleted.`);
    };
    ...
    const value = {
        ...
        technicalInquiries, addTechnicalInquiry, updateTechnicalInquiry, deleteTechnicalInquiry
    };
